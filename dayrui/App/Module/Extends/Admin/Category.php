...
class Category extends \Phpcmf\Table {
    ...
    //protected $_is_admin_min_mode = 0; // 是否是简化模式
    ...
    public function __construct() {
        ...
        $this->_Extend_Init();
        //$this->_is_admin_min_mode = \Phpcmf\Service::M('auth')->is_admin_min_mode()
            //|| (isset($this->admin['setting']['admin_min']) && $this->admin['setting']['admin_min']);
    }
    ...
    public function field_add() {
        ...
        if ($field) {
            foreach ($field as $f) {
                ...
                foreach ($category as $t) {
                    if (isset($t['setting'])
                        && isset($t['setting']['cat_field'])
                        && is_array($t['setting']['cat_field'])
                        && isset($t['setting']['cat_field'][$f['fieldname']])) {
                    } else {
                        $catids[] = $t['id'];
                    }
                }
        ...
        if (IS_POST) {
            ...
            foreach ($category as $t) {
                ...
                foreach ($field as $f) {
                    ...
                    if (isset($post[$fname]) && $post[$fname]) {
                        $catids = dr_string2array($post[$fname]);
            ...
            if ($update) {
                \Phpcmf\Service::M()->db->table($table)->updateBatch($update, 'id');
            }
