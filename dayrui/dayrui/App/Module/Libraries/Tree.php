...
class Tree {
    ...
    public function get_child_row($pid, $onlysub, $is_push) {
        ...
        foreach ($this->data as $t) {
            ...
            if (IS_ADMIN) {
                $ck = 0;
                $rs = \Phpcmf\Hooks::trigger_callback('module_auth_category', $this->mid ? $this->mid : MOD_DIR, $t['id']);
                if ($rs && isset($rs['code'])) {
                    if (!$rs['code']) {
                        continue;
                    } else {
...
<script src="'.ROOT_THEME_PATH.'assets/layui/cascader/cascader'.(IS_XRDEV ? '' : '.min').'.js?v='.CMF_UPDATE_TIME.'"></script>
    ...
    // 递归栏目树形结构
    public function _tree_html($mid, $pid = 0) {
        $this->_tree_html.= '<ul>';
        foreach ($this->_tree as $t) {
            ...
            if (!$ck && dr_is_app('cqx')
                && \Phpcmf\Service::M('content', 'cqx')->is_edit($t['id'], 0, $mid)) {
                continue;
            }
            if ($t['pid'] == $pid) {
                if ($t['tid'] == 1) {
                    ...
                    $url = dr_url('category/edit', ['id' => $t['id'], 'page' => 1]);
                }
                if (!$this->_tree_url) {
                    $this->_tree_url = $url;
                }
    ...
    public function tree_category($mid, $data) {

        $this->_tree = $data;
        $this->_tree_url = '';
        $this->_tree_html = '';
        $this->_tree_html($mid, 0);
        return [$this->_tree_url, $this->_tree_html];
    }
