...
if ($module['category'] && is_array($module['category'])) {
    foreach ($module['category'] as $t) {
        if (!$t['show'] && !$show) {
            ...
            $tt = dr_cat_value($system['site'], $module['mid'], $t['id']);
            if ($tt) {
                $t = $tt;
            }
        }
...
// 上下栏目
if (isset($param['prev']) || isset($param['next'])) {
    $res = [];
    ...
    return $this->_return($system['return'], $res, '');
}
