...
<script>
    function dr_link_url(id, name) {
        dr_iframe_cat('<i class="fa fa-edit"></i> '+name,'<?php echo dr_url(APP_DIR.'/category/link_edit'); ?>&id='+id, '50%', '30%','nogo');
    }
    function dr_content_url(id, name) {
        dr_iframe_cat('<i class="fa fa-edit"></i> '+name,'<?php echo dr_url(APP_DIR.'/category/edit'); ?>&isedit=1&page=1&id='+id, '80%', '90%','nogo');
    }
    // 窗口提交
                ...
                $.ajax({type: "POST",dataType:"json", url: url, data: $(body).find('#myform').serialize(),
                    success: function(json) {
                        ...
                        if (json.code) {
                            ...
                            <?php } else { ?>
                            dr_link_ajax_url('<?php echo dr_url('module/api/update_category_repair'); ?>&is_close=1&all=1&mid=<?php echo $dir; ?>', json);
                            <?php } ?>
                        } else {
    ...
    function dr_cat_field(id, name) {
        dr_iframe('<i class="fa fa-edit"></i> '+name,'<?php echo dr_url(APP_DIR.'/category/field_edit'); ?>&id='+id, '50%', '70%', 'nogo');
    }
    ...
    function dr_tree_data(catid) {
        ...
        if (value == '[+]') {
            $.ajax({
                type: "GET",
                dataType: "json",
                url: "<?php echo dr_url($uriprefix.'/list_index'); ?>&pid="+catid,
                success: function(json) {
                    layer.close(index);
                    if (json.code == 1) {
                        <?php if ($is_total) { ?>
                        $.ajax({
                            type: "POST",
                            dataType: "json",
                            url: "<?php echo dr_url('module/api/ctotal'); ?>",
                            data: {
                                'cid': json.data,
    ...
    $(function() {
        <?php if (defined('SYS_CAT_POPEN') && SYS_CAT_POPEN) {  if (isset($pcats) && is_array($pcats) && $pcats) { $key_ii=-1;$count_ii=dr_count($pcats);foreach ($pcats as $ii) { $key_ii++; $is_first=$key_ii==0 ? 1 : 0;$is_last=$count_ii==$key_ii+1 ? 1 : 0;?>
        ...
        <?php } }  }  if ($is_total) { ?>
            $.ajax({
                ...
                dataType: "json",
                url: "<?php echo dr_url('module/api/ctotal'); ?>",
                data: <?php echo json_encode(['cid'=>$tcats, csrf_token() => csrf_hash()]); ?>,
                success: function(json) {
                    if (json.code == 1) {
                        eval(json.msg);
                    }
...
<div class="note note-danger">
    <p>
        <?php if ($fn_include = $this->_include("category_btn.html")) include($fn_include); ?>
    </p>
</div>
    ...
    <form class="form-horizontal" role="form" id="myform">
        ...
        <div class="bootstrap-table bootstrap-table2">
            <div id="toolbar" class="toolbar">
                ...
                <label><button type="button" onclick="dr_ajax_option('<?php echo dr_url($uriprefix.'/del'); ?>', '<?php echo dr_lang('将同步删除其下级所有栏目和内容，且无法恢复，你确定要删除它们吗？'); ?>', 1)" class="btn red btn-sm"> <i class="fa fa-trash"></i> <?php echo dr_lang('删除'); ?></button></label>
                <?php }  if ($ci->_is_admin_auth('edit')) { ?>
                <label style="margin-right: 5px"><?php echo $move_select; ?></label>
                ...
                <?php if (IS_SHARE && $ci->_is_admin_auth('edit')) { ?>
                <label><div class="btn-group dropdown">
                    <a class="btn  blue btn-sm dropdown-toggle" data-toggle="dropdown" data-hover="dropdown" data-close-others="true" aria-expanded="false" href="javascript:;"> <?php echo dr_lang('批量静态'); ?>
                        <i class="fa fa-angle-down"></i>
                    </a>
                    <ul class="dropdown-menu">
                        <li><a href="javascript:;" onclick="dr_ajax_option('<?php echo dr_url($uriprefix.'/htmlall_edit'); ?>', '<?php echo dr_lang('你确定要批量设置栏目为静态模式吗？'); ?>', 1)" > <?php echo dr_lang('设置栏目静态'); ?></a></li>
                        <li><a href="javascript:;" onclick="dr_ajax_option('<?php echo dr_url($uriprefix.'/phpall_edit'); ?>', '<?php echo dr_lang('你确定要批量设置栏目为动态模式吗？'); ?>', 1)" > <?php echo dr_lang('设置栏目动态'); ?></a></li>
                        <li class="divider"> </li>
                        <li><a href="javascript:;" onclick="dr_ajax_option('<?php echo dr_url($uriprefix.'/htmlall_edit', ['tid'=>1]); ?>', '<?php echo dr_lang('你确定要批量设置内容为静态模式吗？'); ?>', 1)" > <?php echo dr_lang('设置内容静态'); ?></a></li>
                        <li><a href="javascript:;" onclick="dr_ajax_option('<?php echo dr_url($uriprefix.'/phpall_edit', ['tid'=>1]); ?>', '<?php echo dr_lang('你确定要批量设置内容为动态模式吗？'); ?>', 1)" > <?php echo dr_lang('设置内容动态'); ?></a></li>
                    </ul>
                </div></label>
                <?php }  } ?>
