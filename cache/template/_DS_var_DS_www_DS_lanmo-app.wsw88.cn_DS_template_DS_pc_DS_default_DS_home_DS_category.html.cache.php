...
<!--循环输出顶级栏目下面的子栏目及其内容-->
<?php $list_return_c = $this->list_tag("action=category pid=$catid  return=c"); if ($list_return_c && is_array($list_return_c)) extract($list_return_c, EXTR_OVERWRITE); $count_c=dr_count($return_c); if (is_array($return_c) && $return_c) { $key_c=-1;  foreach ($return_c as $c) { $key_c++; $is_first=$key_c==0 ? 1 : 0;$is_last=$count_c==$key_c+1 ? 1 : 0; ?>
    ...
    <div class="col-xl-8 mt-0">
        ...
        <div class="card card-flush h-md-100">
            ...
            <div class="card-header pt-5">
                ...
                <h3 class="card-title align-items-start flex-column">
                    <div class="d-flex align-items-center mb-2">
                        <!--begin::Currency-->
                        <a href="<?php echo $c['url']; ?>" class="fs-5 fw-bold text-gray-800 "><?php echo $c['name']; ?></span>
                            <!--end::Currency-->
                    </div>
                </h3>
                <!--end::Title-->
            ...
            <div class="card-body pt-6">
                <!--分页显示列表数据-->
                <?php $list_return = $this->list_tag("action=module catid=$c[id] order=updatetime num=3"); if ($list_return && is_array($list_return)) extract($list_return, EXTR_OVERWRITE); $count=dr_count($return); if (is_array($return) && $return) { $key=-1; foreach ($return as $t) { $key++; $is_first=$key==0 ? 1 : 0;$is_last=$count==$key+1 ? 1 : 0; ?>
                <div class=" d-flex align-items-center  p-lg-3">
                    ...
                    <div class="ms-6">
                        ...
                        <div class="article-tag">
                            <?php $kws=dr_get_content_kws($t['keywords']);  if ($kws) {  $kws=dr_arraycut($kws, 3);  if (isset($kws) && is_array($kws) && $kws) { $key_url=-1;$count_url=dr_count($kws);foreach ($kws as $name=>$url) { $key_url++; $is_first=$key_url==0 ? 1 : 0;$is_last=$count_url==$key_url+1 ? 1 : 0; ?>
    ...
    <div class="col-xl-4 mt-0">
        ...
        <div class="card card-flush h-md-100">
            ...
            <div class="card-body pt-3">
                ...
                <?php $list_return = $this->list_tag("action=module catid=$c[id] order=hits num=4"); if ($list_return && is_array($list_return)) extract($list_return, EXTR_OVERWRITE); $count=dr_count($return); if (is_array($return) && $return) { $key=-1; foreach ($return as $t) { $key++; $is_first=$key==0 ? 1 : 0;$is_last=$count==$key+1 ? 1 : 0; ?>
                <!--begin::Item-->
